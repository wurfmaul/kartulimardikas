...
CREATE TABLE IF NOT EXISTS `algorithm` (
  `aid` int(8) unsigned NOT NULL AUTO_INCREMENT COMMENT 'algorithm id',
  `uid` int(6) unsigned NOT NULL COMMENT 'the author''s user id',
  ...
  `original` int(8) unsigned DEFAULT NULL COMMENT 'If the algorithm was cloned, id of the original.',
  PRIMARY KEY (`aid`),
  CONSTRAINT FOREIGN KEY (`uid`) REFERENCES `user`(`uid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT FOREIGN KEY (`original`) REFERENCES `algorithm`(`aid`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;

CREATE TABLE IF NOT EXISTS `tag` (
  `tag` varchar(64) COLLATE utf8_general_ci NOT NULL COMMENT 'The tag''s name',
  `aid` int(8) unsigned NOT NULL COMMENT 'The algorithm''s id',
  PRIMARY KEY (`tag`,`aid`),
...
INSERT INTO `user` (`username`, `email`, `password`, `language`, `rights`, `date_registration`) VALUES
  ('admin', 'admin@domain.org', '$2y$10$fJHBSe6cHlE56h2v7qmWauwmznkNji1eZYxXkZh.z6rnXJ/f.4Ov2', 'en', 2, CURRENT_TIMESTAMP);